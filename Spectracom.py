    ...
    def scenario_reading(self):
        ...
        for section in range(len(scenario)-2):
            ...
            if scenario[section][3] == '':
                ...
                precision = 0.00006
                while ((SpectracomGetData(self.device).get_current_pos()[0][1] not in interval.Interval.between(float(scenario[section][0]) -
                        precision, float(scenario[section][0]) + precision)) and
                        ((SpectracomGetData(self.device).get_current_pos()[0][2] not in interval.Interval.between(float(scenario[section][0]) -
                         precision, float(scenario[section][0]) + precision)))):
                    SpectracomCommand(self.device).set_heading(tools.heading_compute(SpectracomGetData(self.device).get_current_pos()[0][1], (float(scenario[section][0])),
                ...
                if (scenario[section][0] != '') or (scenario[section][1] != '') or (scenario[section][2] != ''):
                    SpectracomCommand(self.device).set_position(float(scenario[section][0]), float(scenario[section][1]),
